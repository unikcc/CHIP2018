...
class SoftmaxAttention_Decompose(nn.Module):
    """
    Attention layer taking premises and hypotheses encoded by an RNN as input
    and computing the soft attention between their elements.
    ...
    def forward(self,
        ...
        component_hyp_1 = (attended_hypotheses * hypothesis_batch).sum(2)
        component_hyp_2 = mask_fenmu((attended_hypotheses * attended_hypotheses).sum(2), hypothesis_mask)
        component_hyp = (component_hyp_1 / component_hyp_2).unsqueeze(2) * attended_hypotheses

        component_hyp_un = hypothesis_batch - component_hyp

        component_pre_1 = (attended_premises * premise_batch).sum(2)
        component_pre_2 = mask_fenmu((attended_premises * attended_premises).sum(2), premise_mask)
        component_pre = (component_pre_1 / component_pre_2).unsqueeze(2) * attended_premises
        component_pre_un = premise_batch - component_pre
        #return attended_premises, attended_hypotheses
        return component_pre, component_hyp, component_pre_un, component_hyp_un

def mask_fenmu(vector, mask):
    vector += (1-mask) * 1e12
    return vector
